...
MoviesMenager	  movies_manager;
OMXReader         m_omx_reader_next;
...
void * reader_open_thread(void *data) {
  char *filename = (char *) data;
  if(!m_omx_reader.Open(filename, m_dump_format, m_config_audio.is_live, m_timeout, m_cookie.c_str(), m_user_agent.c_str(), m_lavfdopts.c_str(), m_avdict.c_str())) {
    Logger::err("Cannot open file " + AnsiString(filename));
  }
}
...
int main(int argc, char *argv[])
{
  ...
  if(argc < 3)
    print_usage();
  movies_manager = MoviesMenager();
  movies_manager.loadMovies(argv[1], argv[2]);
...
play_file:
  ...
  reader_open_thread((void*)m_filename.c_str());

  if (m_dump_format_exit)
    Logger::err("m_dump_format_exit fail");
  ...
  if(!m_player_init) {
    ...
    if(!m_av_clock->OMXInitialize())
      Logger::err("m_av_clock->OMXInitialize() fail");

    if(m_config_video.hdmi_clock_sync && !m_av_clock->HDMIClockSync())
      Logger::err("m_config_video.hdmi_clock_sync && !m_av_clock->HDMIClockSync() fail");
    ...
    if(m_has_video && !m_player_video.Open(m_av_clock, m_config_video))
      Logger::err("m_player_video.Open fail");
    ...
    if(m_has_subtitle || m_osd)
    {
      ...
      if(!m_player_subtitles.Open(m_omx_reader.SubtitleStreamCount(),
                                  ...
                                  m_av_clock))
        Logger::err("m_player_subtitles.Open fail");
    }
    ...
    if(m_has_audio && !m_player_audio.Open(m_av_clock, m_config_audio, &m_omx_reader)) {
      Logger::err("m_player_audio.Open fail");
    }
